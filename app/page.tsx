...
export default function Home() {
  ...
  useEffect(() => {
    setListItems(
      JSON.parse(window.localStorage.getItem("TODO_LIST_ITEMS") || "[]")
    );
  }, []);
  useEffect(() => {
  ...
  const addUpdateItemToList = (item: string) => {
    if (currentItem?.id) {
      const index: number = listItems.findIndex(
  ...
  const handleDeleteClick = (todoItem: TodoItem) => {
    ...
    if (confirmation) {
      const updatedItems: TodoItem[] = listItems.filter(
  ...
  const markItemAsCompleted = (event: any, todo: TodoItem) => {
    if (
      ...
      event.target.classList.contains("un-completed")
    ) {
      const userConsent = window.confirm(
        `Are you sure you want to Mark this item as ${
          todo.isCompleted ? "Un Completed" : "Completed"
        } ?`
      );
      if (userConsent) {
        todo.isCompleted = !todo.isCompleted;
        setListItems([
          ...listItems.filter((item) => item.id !== todo.id),
          todo,
        ]);
      }
    }
  ...
  return (
    <div className="todo-wrapper">
      <h1>{title ? title : "Add tasks (Not from env)"}</h1>
      <AddUpdateTodo
        addUpdateItemToList={addUpdateItemToList}
        currentItem={currentItem}
      />
      {listItems.sort().map((todo, index) => (
        <div
          onClick={(e) => markItemAsCompleted(e, todo)}
          className="todo-item"
          key={index}
        >
          <p className={`${todo.isCompleted ? "completed" : "un-completed"}`}>
            {todo.item}
          </p>
          <div>
            <FontAwesomeIcon
              icon={faPenToSquare}
              onClick={(e) => handleEditClick(todo)}
            />
            <FontAwesomeIcon
              onClick={(e) => handleDeleteClick(todo)}
              icon={faTrash}
            />
          </div>
        </div>
